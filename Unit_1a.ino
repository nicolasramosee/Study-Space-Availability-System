...
void setup()
{
  ...
  sensorRL.setResolution(imageWidth * imageWidth); // 8×8 grid
  sensorRL.setRangingFrequency(7);                // 7 Hz
  sensorRL.startRanging();
}
...
void loop()
{
  ...
  if (sensorRL.isDataReady() && sensorRL.getRangingData(&measurementData))
  {
...
void sendToFirebase(int motionStatus)
{
  ...
  String        url     = "/1a/LiDAR.json?auth=" + String(firebaseSecret);
